...
public class Hermione extends Ability {
    ...
    public Hermione(String playerName)
    {
        super(playerName,"헤르미온느", 133, true, true, false, des);
            ...
            if(PlayerInventory.ItemCheck(p, co, sta1) && CoolTimeChecker.Check(p, 1)){
                switch(event.getMessage()){
                    ...
                    case "옵스큐로": case "Obscuro":
                        ...
                        Skill.Use(p, co, sta1, 1, cool1);
                        for(Player e: target){
                            e.sendMessage(ChatColor.GRAY+"옵스큐로"+ChatColor.WHITE+"에 당했습니다!");
    ...
    private void Stupefy(Player p){
        ...
        for(Player e:list){
            if(r.nextBoolean()){
                e.addPotionEffect((new PotionEffect(PotionEffectType.SLOW, 20*8,2)));
            }
        }
    }
    ...
    private void Expelliarmus(Player p){
        ...
        if(Bukkit.getPlayer(abilitytarget)==null){
            p.sendMessage("타겟으로 등록하신 분이 지금은 서버에 없는 것 같습니다.");
        }else{
    ...
    private void AvadaKedavra(Player p){
        ...
        if(abilitytarget==null){
            p.sendMessage("타겟으로 지정된 사람이 없습니다!");
            return;
        }
        Player target=Bukkit.getPlayer(abilitytarget);
        if(target==null){
