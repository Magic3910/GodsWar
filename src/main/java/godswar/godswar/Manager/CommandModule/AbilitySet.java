...
{
    public static void Module(CommandSender sender, String[] data, Theomachy t)
    {
        if (PermissionChecker.Sender(sender))
        {
                ...
                else if (data[1].equalsIgnoreCase("get"))//삭제
                {
                    if (data[2] != null) {
                        Ability ability = GameData.PlayerAbility.get(Bukkit.getPlayer(data[2]).getName());
                        if (ability.abilityCode == 133 || ability.abilityCode == 134 || ability.abilityCode == 124)
                            ability.conditionSet();
                    }
                    else
                        sender.sendMessage("능력 아이템을 지급할 플레이어의 이름을 적어주세요.");
                }
    ...
    public static void Rns(String playerName, Theomachy t)
    {
        if (RandomNumberConstructor.CanlistDump.isEmpty()) {
            Bukkit.getPlayer(playerName).sendMessage("능력이 모두 소진되어 배정받지 못했습니다");
        }
        else {
            Theomachy.log.info("남은 능력 수 : "+ RandomNumberConstructor.CanlistDump.size());
    ...
    private static void RandomAssignment(CommandSender p, Theomachy t)
    {
    ...
    private static void forceAssignment(CommandSender sender, String[] data, Theomachy t)
    {
    ...
    public static void abiltiyAssignment(int abilityCode, String playerName, CommandSender p, Theomachy t)
    {
        if (abilityCode == 1)
            ...
            GameData.PlayerAbility.put(playerName, new Chaos(playerName));
        ...
        else if (abilityCode == 1006)
            GameData.PlayerAbility.put(playerName, new Jellyfish(playerName));
        else if (abilityCode == 1007)
            GameData.PlayerAbility.put(playerName, new Magician(playerName));
        else if (abilityCode == 1008)
            GameData.PlayerAbility.put(playerName, new Flora(playerName));
        else if (abilityCode == 1009)
